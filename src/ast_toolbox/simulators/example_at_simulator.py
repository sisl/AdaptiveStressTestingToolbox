...
class ExampleATSimulator(ASTSimulator):
    ...
    def __init__(self, **kwargs):
        ...
        self.use_01_formulation = False # Use the [0, 1] representation of the action space. Note, duplicate in "ExampleATSpaces". (testing)
        self.use_stl_robustness = True # Use STL robustness instead of direct miss distance. (testing)
        self.use_end_action_interpolation = False # append action out of time range to allow linear interpolation (from MATLAB documentation)
    ...
    def get_first_action(self):
        if self.use_end_action_interpolation:
            # NOTE: Based on the MATLAB documentation, this is to ensure linear interpolation occurs (https://www.mathworks.com/help/simulink/slref/modeling-an-automatic-transmission-controller.html)
    ...
    def is_goal(self):
        ...
        if self._benchmark == "AT1":
            ...
            error("No benchmark named ", self._benchmark)
        ...
        if self._reject_time_inconsistency:
            ...
            if self.use_stl_robustness:
                if self._benchmark == "AT1":
                    ...
                    error("observe(), no benchmark named ", self._benchmark)
            else:
                if self._benchmark == "AT1":
                    self._miss_distance = min(self.robustness(0, self._at1_max_time, lambda s: self._at1_speed_thresh - s, self._times, self._speeds)) # NOTE: (thresh - s) so negative miss is past event.
                elif self._benchmark == "AT2":
                    self._miss_distance = min(self.robustness(0, self._at2_max_time, lambda w: self._at2_rpm_thresh - w, self._times, self._rpms)) # NOTE: (thresh - s) so negative miss is past event.
                else:
                    error("observe(), no benchmark named ", self._benchmark)
